<?php
require_once('header.php');		
?>

<style>
body { 
    background-image:url('http://localhost/wellness/uiux/images/joina.PNG');
    background-color:#F8F8F8;
    background-size: auto auto;
    background-repeat:no-repeat;
    background-position: center;
}
</style>

<body>
<div class="container">    
        <div id="loginbox" style="margin-top:180px;" class="mainbox col-md-6 col-md-offset-3 col-sm-8 col-sm-offset-2">                    
            <div class="panel panel-info" >
                    <div class="panel-heading">
                        <div class="panel-title">Welcome to the Wellness Application. PLease Login.</div>
                    </div>     

                    <div style="padding-top:30px" class="panel-body" >

                        <div style="display:none" id="login-alert" class="alert alert-danger col-sm-12"></div>
                            
                        <form id="loginform" class="form-horizontal" role="form" action="http://localhost/wellness/home/" method="post">  
                            <div style="margin-bottom: 25px" class="input-group">
                                        <span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span>
                                        <input type="text" class="form-control" name="userid" placeholder="User ID" autofocus value="<?php echo set_value('userid')?>">
		<font color="red"><?php echo form_error('userid'); ?></font>                                       
                                    </div>
                                
                            <div style="margin-bottom: 25px" class="input-group">
                                        <span class="input-group-addon"><i class="glyphicon glyphicon-lock"></i></span>
                                        <input type="password" class="form-control" name="password" placeholder="Password" value="<?php echo set_value('password')?>">
		<font color="red"><?php echo form_error('password'); ?></font>
                                    </div>
                                    

                                
                            <div class="input-group">
                                      <div class="checkbox">
                                        <label>
                                          <input id="login-remember" type="checkbox" name="remember" value="1"> Remember me
                                        </label>
                                      </div>
                                    </div>


                                <div style="margin-top:10px" class="form-group">
                                    <!-- Button -->

                                    <div class="col-sm-12 controls">
        <button class="btn btn-lg btn-success btn-block" type="submit">Login</button>
		<h5 align="center"><font color="red"><?php if(isset($no_user)){echo $no_user;}?></font></h5>

                                    </div>
                                </div>


                                <div class="form-group">
                                    <div class="col-md-12 control">
                                        <div style="border-top: 1px solid#888; padding-top:15px; font-size:85%" >
                                            &copy; Rapha Healthcare Systems.
                                        </div>
                                    </div>
                                </div>    
                            </form>     



                        </div>                     
                    </div>  
        </div>
        

               
               
                
         </div> 
<?php
require_once('footer.php');
?>